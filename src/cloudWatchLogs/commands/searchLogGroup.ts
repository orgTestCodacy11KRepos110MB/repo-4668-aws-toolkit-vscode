...
import { isValidResponse, isWizardControl, Wizard, WIZARD_EXIT, WIZARD_RETRY } from '../../shared/wizards/wizard'
...
export class SearchPatternPrompter extends InputBoxPrompter {
    ...
    protected async promptUser(): Promise<PromptResult<string>> {
        ...
        try {
            ...
            const validationResult = await this.validateSearchPattern(this.inputBox.value, true)
    ...
    async validateSearchPattern(searchPattern: string, isFinalInput: boolean): Promise<string | undefined> {
        if (!isFinalInput) {
