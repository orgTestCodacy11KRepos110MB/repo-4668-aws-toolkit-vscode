...
describe('CollectionUtils', async function () {
    ...
    describe('toStream', function () {
        it('yields values that resolve first regardless of order', async function () {
            const values = [0, 1, 2, 3].reduce(cons, [] as Promise<number>[])
    ...
    describe('join', function () {
        async function* toAsyncIterable<T>(iterable: Iterable<Promise<T>>, returnValue?: T) {
            for await (const val of iterable) {
                yield val
            }
        ...
        async function run<T>(testCase: TestCase<T>) {
            ...
            const arr = testCase.data.reduce(cons, [] as Promise<T>[])
        ...
        it('throws if an iterable fails', async function () {
            const err = new Error()
            const arr = [0, 1, 2, 3].reduce(cons, [] as Promise<number>[])
        ...
        it('ignores values returned by the iterables', async function () {
            const expected = [0, 1, 2, 3]
            const arr = expected.reduce(cons, [] as Promise<number>[])
